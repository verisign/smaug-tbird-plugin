...
function onLoad() {
  ...
  try {
    ...
    var keyId = null;
    var fingerprint = "";
    var sigListStr = smaugSvc.getKeySig("0x"+window.arguments[0].keyId, exitCodeObj, errorMsgObj);
    ...
    if (exitCodeObj.value == 0) {
      ...
      for (i=0; i < sigList.length; i++) {
        ...
        case "fpr":
          if (fingerprint=="") {
            Ec.DEBUG_LOG("smaugSignKeyDlg.js: fpr:"+currKey+" -> "+aLine[9]+"\n");
            fingerprint = aLine[9];
          }
          break;
        default:
...
function smgKeySelCb() {
  ...
  var acceptButton = document.getElementById("smaugSignKeyDlg").getButton("accept");
