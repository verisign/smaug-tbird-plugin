...
SmgInitCommon("smaugKeyDetailsDlg");
...
function onLoad() {
  ...
  if (window.arguments[0].secKey) {
    setAttr("keyType", SmgGetString("keyTypePair"));
    document.getElementById("ownKeyCommands").removeAttribute("hidden");
  }
  else {
    setAttr("keyType", SmgGetString("keyTypePublic"));
  }
}
...
function reloadData() {
  ...
  SmgCleanGuiList(uidList);

  var sigListStr = smaugSvc.getKeySig("0x"+gKeyId, exitCodeObj, errorMsgObj);
  if (exitCodeObj.value == 0) {
    var keyDetails = SmgGetKeyDetails(sigListStr);
    ...
    for (var i=0; i < keyDetails.subkeyList.length; i++) {
      SmgAddSubkey(treeChildren, keyDetails.subkeyList[i]);
    }
    ...
    if (keyDetails.fingerprint) {
      setAttr("fingerprint", SmgFormatFpr(keyDetails.fingerprint));
    }
  }
}
...
function createUidRow(aLine) {
  ...
  var uidCol = createCell(SmgConvertGpgToUnicode(aLine[9]));
  var validCol = createCell(getTrustLabel(aLine[1]));
  if ("dre".search(aLine[1]) >= 0) {
    uidCol.setAttribute("properties", "smgKeyInactive");
    validCol.setAttribute("properties", "smgKeyInactive");
  }
...
function getTrustLabel(trustCode) {
  var trustTxt=SmgGetTrustLabel(trustCode);
  if (trustTxt=="-" || trustTxt.length==0) {
    trustTxt=SmgGetString("keyValid.unknown");
  }
  return trustTxt;
}
...
function signKey() {
  if (SmgSignKey(gUserId, gKeyId, null)) {
    enableRefresh();
    reloadData();
  }
}
...
function changeExpirationDate() {
  if (SmgEditKeyExpiry([gUserId], [gKeyId])) {
    enableRefresh();
    reloadData();
  }
}
...
function setOwnerTrust() {
  ...
  if (SmgEditKeyTrust([gUserId], [gKeyId])) {
    enableRefresh();
    reloadData();
  }
}
...
function changePassword() {
  SmgChangeKeyPwd(gKeyId, gUserId);
}

function revokeKey() {
  SmgRevokeKey(gKeyId, gUserId, function _revokeKeyCb(success) {
    if (success) {
      enableRefresh();
      reloadData();
    }
...
function genRevocationCert() {
  SmgCreateRevokeCert(gKeyId, gUserId);
}
