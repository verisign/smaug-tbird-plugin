...
function onLoad() {
  ...
  Ec.DEBUG_LOG("smgRetrieveProgress: onLoad\n");
...
function processEnd (progressBar, exitCode) {
  Ec.DEBUG_LOG("smaugRetrieveProgress.js: processEnd\n");

  if (gProcess) {
    gProcess = null;
    Ec.DEBUG_LOG("smaugRetrieveProgress.js: processEnd: exitCode = "+exitCode+"\n");

    var statusText=gSmgCallbackFunc(exitCode, "", false);
    ...
    Ec.DEBUG_LOG("smaugRetrieveProgress.js: processEnd: errorMsg="+errorMsg);
    if (errorMsg.search(/ec=\d+/i)>=0) {
      exitCode=-1;
    }
    statusText=gSmgCallbackFunc(exitCode, "", false);
    gSmgCallbackFunc(exitCode, errorMsg, true);
    if (exitCode == 0) {
...
function smgSendKeyCancel() {
  if (gProcess) {
    var p = gProcess;
    gSmgCallbackFunc=null;
    gProcess=null;
    p.kill(false);
  }
}
