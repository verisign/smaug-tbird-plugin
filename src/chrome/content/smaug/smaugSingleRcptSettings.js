...
SmgInitCommon("smaugSingleRcptSettings");
...
*/
  ...
  if (typeof(window.arguments[INPUT].keyId)=="object") {
    ...
    case "":
      ...
      break;
    default:
      smgSetKeys(window.arguments[INPUT].keyId);
      action="actionUseKey";
    }
  }
  else {
  ...
  smgEnableKeySel(action=="actionUseKey");
...
function smaugDlgOnAccept() {
  ...
  if (ruleEmail.value.length==0) {
    SmgAlert(SmgGetString("noEmptyRule"));
    return false;
  }
  if (ruleEmail.value.search(/[\<\>]/)>=0) {
    SmgAlert(SmgGetString("invalidAddress"));
    return false;
  }
  if (ruleEmail.value.search(/[{}]/)>=0) {
    SmgAlert(SmgGetString("noCurlyBrackets"));
    return false;
  }
  ...
  case 2:
    if (keyList == "" && (window.arguments[RESULT].encrypt>0)) {
      if (!SmgConfirm(SmgGetString("noEncryption", ruleEmail.value, ruleEmail.value))) {
        return false;
      }
...
function smaugDlgKeySelection() {
  DEBUG_LOG("smaugMsgComposeHelper.js: smaugDlgKeySelection: \n");
...
function smgSetKeys(keyList) {
  ...
  if ((keyList.length==0) || (keyList.length==1 && keyList[0].length==0)) {
    ...
    var userListTxt = smaugSvc.getUserIdList(false,
                                               false,
                                               exitCodeObj,
                                               statusFlagsObj,
                                               errorMsgObj);
    ...
    for (var i=0; i<keyList.length; i++) {
      ...
      if(keyList[i].indexOf("GROUP:") == 0) {
        encryptionList.appendItem(keyList[i].substr(6)+" "+SmgGetString("keyTrust.group"), keyList[i]);
      }
      else
        encryptionList.appendItem("0x"+keyList[i].substr(10,8)+" ("+SmgConvertGpgToUnicode(userDesc)+")",
                                keyList[i]);
    }
  }
}

function smgEnableKeySel(enable) {
  if (enable) {
