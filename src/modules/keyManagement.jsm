...
var EXPORTED_SYMBOLS = [ "SmaugKeyMgmt" ];
...
KeyEditor.prototype = {
  ...
  processLine: function(txt) {
    ...
    try {
      ...
      if (txt.indexOf("[GNUPG:] SMAUG_FAILURE")==0) {
...
var SmaugKeyMgmt = {
  ...
  editKey: function (parent, needPassphrase, userId, keyId, editCmd, inputData, callbackFunc, requestObserver, parentCallback) {
    ...
    var smaugSvc = Ec.getService(parent);
    if (!smaugSvc) {
      Ec.ERROR_LOG("keyManagmenent.jsm: Smaug.editKey: not yet initialized\n");
    ...
    var command= smaugSvc.agentPath;
    ...
    try {
      var proc = subprocess.call({
        ...
        stdout: function(data) {
          ...
          Ec.DEBUG_LOG("keyManagmenent.jsm: Smaug.editKey: GnuPG terminated with code="+result.exitCode+"\n");
  ...
  setKeyTrust: function (parent, keyId, trustLevel, callbackFunc) {
    Ec.DEBUG_LOG("keyManagmenent.jsm: Smaug.setKeyTrust: trustLevel="+trustLevel+", keyId="+keyId+"\n");
  ...
  setKeyExpiration: function (parent, keyId, subKeys, expiryLength, timeScale, noExpiry, callbackFunc) {
    Ec.DEBUG_LOG("keyManagmenent.jsm: Smaug.setKeyExpiry: keyId="+keyId+"\n");
  ...
  signKey: function (parent, userId, keyId, signLocally, trustLevel, callbackFunc) {
    Ec.DEBUG_LOG("keyManagmenent.jsm: Smaug.signKey: trustLevel="+trustLevel+", userId="+userId+", keyId="+keyId+"\n");
  ...
  genRevokeCert: function (parent, keyId, outFile, reasonCode, reasonText, callbackFunc) {
    Ec.DEBUG_LOG("keyManagmenent.jsm: Smaug.genRevokeCert: keyId="+keyId+"\n");
  ...
  addUid: function (parent, keyId, name, email, comment, callbackFunc) {
    Ec.DEBUG_LOG("keyManagmenent.jsm: Smaug.addUid: keyId="+keyId+", name="+name+", email="+email+"\n");
  ...
  deleteKey: function (parent, keyId, deleteSecretKey, callbackFunc) {
    Ec.DEBUG_LOG("keyManagmenent.jsm: Smaug.addUid: keyId="+keyId+", deleteSecretKey="+deleteSecretKey+"\n");
  ...
  changePassphrase: function (parent, keyId, oldPw, newPw, callbackFunc) {
    Ec.DEBUG_LOG("keyManagmenent.jsm: Smaug.changePassphrase: keyId="+keyId+"\n");

    var pwdObserver = new ChangePasswdObserver();
    var r= this.editKey(parent, false, null, keyId, "passwd",
                        { oldPw: oldPw,
                          newPw: newPw,
                          useAgent: Ec.smaugSvc.useGpgAgent(),
                          step: 0,
  ...
  enableDisableKey: function (parent, keyId, disableKey, callbackFunc) {
    Ec.DEBUG_LOG("keyManagmenent.jsm: Smaug.enableDisableKey: keyId="+keyId+", disableKey="+disableKey+"\n");
  ...
  setPrimaryUid: function (parent, keyId, idNumber, callbackFunc) {
    Ec.DEBUG_LOG("keyManagmenent.jsm: Smaug.setPrimaryUid: keyId="+keyId+", idNumber="+idNumber+"\n");
  ...
  deleteUid: function (parent, keyId, idNumber, callbackFunc) {
    Ec.DEBUG_LOG("keyManagmenent.jsm: Smaug.deleteUid: keyId="+keyId+", idNumber="+idNumber+"\n");
    var r = this.editKey(parent, true, null, keyId, "",
                        { idNumber: idNumber,
                          step: 0,
                          usePassphrase: true },
  ...
  revokeUid: function (parent, keyId, idNumber, callbackFunc) {
    Ec.DEBUG_LOG("keyManagmenent.jsm: Smaug.revokeUid: keyId="+keyId+", idNumber="+idNumber+"\n");
    var r = this.editKey(parent, true, null, keyId, "",
                        { idNumber: idNumber,
                          step: 0,
                          usePassphrase: true },
  ...
  addPhoto: function (parent, keyId, photoFile, callbackFunc) {
    Ec.DEBUG_LOG("keyManagmenent.jsm: Smaug.addPhoto: keyId="+keyId+"\n");
  ...
  genCardKey: function (parent, name, email, comment, expiry, backupPasswd, requestObserver, callbackFunc) {
    Ec.DEBUG_LOG("keyManagmenent.jsm: Smaug.genCardKey: \n");
    var generateObserver = new smgCardAdminObserver(requestObserver, Ec.isDosLike());
  ...
  cardAdminData: function (parent, name, firstname, lang, sex, url, login, forcepin, callbackFunc) {
    Ec.DEBUG_LOG("keyManagmenent.jsm: Smaug.cardAdminData: parent="+parent+", name="+name+", firstname="+firstname+", lang="+lang+", sex="+sex+", url="+url+", login="+login+", forcepin="+forcepin+"\n");
    var adminObserver = new smgCardAdminObserver(null, Ec.isDosLike());
    var r = this.editKey(parent, false, null, "", ["--with-colons", "--card-edit"],
            { step: 0,
              name: name,
              firstname: firstname,
  ...
  cardChangePin: function (parent, action, oldPin, newPin, adminPin, pinObserver, callbackFunc) {
    Ec.DEBUG_LOG("keyManagmenent.jsm: Smaug.cardChangePin: parent="+parent+", action="+action+"\n");
    var adminObserver = new smgCardAdminObserver(pinObserver, Ec.isDosLike());
    var smaugSvc = Ec.getService(parent);

    var r = this.editKey(parent, smaugSvc.useGpgAgent(), null, "", ["--with-colons", "--card-edit"],
            { step: 0,
              pinStep: 0,
              cardAdmin: true,
              action: action,
...
function smgCardAdminObserver(guiObserver, isDosLike) {
  this._guiObserver = guiObserver;
  this._isDosLike = isDosLike;
}

smgCardAdminObserver.prototype =
{
  _guiObserver: null,
  _failureCode: 0,
  ...
  QueryInterface : function(iid)
  {
    if (iid.equals(Ci.nsISmgMimeReadCallback) ||
  ...
  onDataAvailable: function (data) {
    var ret="";
    Ec.DEBUG_LOG("keyManagmenent.jsm: smgCardAdminObserver.onDataAvailable: data="+data+"\n");
