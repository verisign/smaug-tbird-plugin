...
var EXPORTED_SYMBOLS = [ "SmaugCommon" ];
...
// <EMO>
const SMG_EXTENSION_GUID = "{847b3a00-7ab1-11d4-8f02-999999999999}";
// </EMO>
...
const SMAUG_PREFS_ROOT = "extensions.smaug.";
...
var gSmgExtensionVersion;
var gSmgInstallLocation;
...
const GPG_COMMENT_OPT = "Using GnuPG with %s - http://www.smaug.net/";
...
var SmaugCommon = {
  ...
  SMAUG_CONTRACTID: "@mozdev.org/smaug/smaug;1",
  ...
  // - see also smaugEncryptionDlg.xul
  SMG_NEVER:     0,
  SMG_UNDEF:     1,
  SMG_ALWAYS:    2,
  SMG_CONFLICT: 99,
  ...
  SMG_FINAL_NO:        0,
  SMG_FINAL_YES:       1,
  SMG_FINAL_FORCENO:  10,
  SMG_FINAL_FORCEYES: 11,
  SMG_FINAL_CONFLICT: 99,

  // variables
  smaugSvc: null,
  smgStringBundle: null,
  statusFlags: gStatusFlags,
  prefBranch: null,
  prefRoot: null,
  prefService: null,
  envList: null, // currently filled from smaugCommon.jsm
  ...
  getService: function (win, startingPreferences) {
    ...
    if (this.smaugSvc) {
      return this.smaugSvc.initialized ? this.smaugSvc : null;
    }

    try {
      this.smaugSvc = Cc[this.SMAUG_CONTRACTID].createInstance(Ci.nsIEnigmail);
    }
    catch (ex) {
      this.ERROR_LOG("smaugCommon.jsm: Error in instantiating SmaugService\n");
      return null;
    }

    this.DEBUG_LOG("smaugCommon.jsm: this.smaugSvc = "+this.smaugSvc+"\n");

    if (!this.smaugSvc.initialized) {
      // Initialize smaug

      var firstInitialization = !this.smaugSvc.initializationAttempted;
      ...
      try {
        // Initialize smaug
        this.smaugSvc.initialize(win, this.getVersion(), this.prefBranch);
        ...
        if (firstInitialization) {
          ...
          var errMsg = this.smaugSvc.initializationError ? this.smaugSvc.initializationError : this.getString("accessError");

          errMsg += "\n\n"+this.getString("avoidInitErr");

          var checkedObj = {value: false};
          if (this.getPref("initAlert")) {
            var r = this.longAlert(win, "Smaug: "+errMsg,
          ...
          if (this.getPref("initAlert")) {
            this.smaugSvc.initializationAttempted = false;
            this.smaugSvc = null;
          }
        }
      ...
      this.DEBUG_LOG("smaugCommon.jsm: getService: "+configuredVersion+"\n");

      if (firstInitialization && this.smaugSvc.initialized &&
          this.smaugSvc.agentType && this.smaugSvc.agentType == "pgp") {
        this.alert(win, this.getString("pgpNotSupported"));
      }

      if (this.smaugSvc.initialized && (this.getVersion() != configuredVersion)) {
        ConfigureSmaug(win, startingPreferences);
      }
    }

    if (this.smaugSvc.logFileStream) {
      gLogLevel = 5;
    }
  ...
  getVersion: function()
  {
    this.DEBUG_LOG("smaugCommon.jsm: getVersion\n");

    var addonVersion = gSmgExtensionVersion;

    this.DEBUG_LOG("smaugCommon.jsm: installed version: "+addonVersion+"\n");
    return addonVersion;
  },
  ...
  savePrefs: function ()
  {
    this.DEBUG_LOG("smaugCommon.js: savePrefs\n");
    try {
  ...
  initPrefService: function() {
    ...
    try {
      ...
      this.prefBranch      = this.prefService.getBranch(SMAUG_PREFS_ROOT);
      ...
      this.ERROR_LOG("smaugCommon.jsm: Error in instantiating PrefService\n");
  ...
  getPref: function (prefName)
  {
    ...
    try {
      ...
      this.ERROR_LOG("smaugCommon.jsm: getPref: unknown prefName:"+prefName+" \n");
   }
  ...
  {
     this.DEBUG_LOG("smaugCommon.jsm: setPref: "+prefName+", "+value+"\n");
  ...
  {
    gPromptSvc.alert(win, this.getString("smgAlert"), mesg);
  },
  ...
  longAlert: function (win, mesg, checkBoxLabel, okLabel, labelButton2, labelButton3, checkedObj)
  {
    ...
    win.openDialog("chrome://smaug/content/smaugAlertDlg.xul", "",
  ...
  promptValue: function (win, mesg, valueObj)
  {
    ...
    return gPromptSvc.prompt(win, this.getString("smgPrompt"),
  ...
  {
    this.DEBUG_LOG("smaugCommon.jsm: getFrame: name="+frameName+"\n");
  ...
  {
    this.DEBUG_LOG("smaugCommon.jsm: filePicker: "+save+"\n");
  ...
  {
    this.DEBUG_LOG("smaugCommon.jsm: newStreamListener\n");
    ...
    var simpleStreamListener = {
      ...
      {
        // SmaugCommon.DEBUG_LOG("smaugCommon.jsm: stringListener.onStartRequest\n");
      },

      onStopRequest: function (channel, ctxt, status)
      {
        // SmaugCommon.DEBUG_LOG("smaugCommon.jsm: stringListener.onStopRequest: "+ctxt+"\n");
        this.inStream = null;
        var cbFunc = this._onStopCallback;
        var cbData = this.data;

        SmaugCommon.setTimeout(function _cb() {
          cbFunc(cbData);
        });
      },
      ...
      onDataAvailable: function(req, sup, stream, offset, count)
      {
        ...
        // SmaugCommon.DEBUG_LOG("smaugCommon.jsm: stringListener.onDataAvailable: "+count+"\n");
  ...
  newRequestObserver: function (terminateFunc, terminateArg)
  {
    ...
    requestObserver.prototype = {
      ...
      onStartRequest: function (channel, ctxt)
      {
        SmaugCommon.DEBUG_LOG("smaugCommon.jsm: requestObserver.onStartRequest\n");
      },

      onStopRequest: function (channel, ctxt, status)
      {
        SmaugCommon.DEBUG_LOG("smaugCommon.jsm: requestObserver.onStopRequest: "+ctxt+"\n");
        SmaugCommon.dispatchEvent(this._terminateFunc, 0, [ this._terminateArg ]);
      }
    };
  ...
  WRITE_LOG: function (str)
  {
    ...
    if (this.smaugSvc && this.smaugSvc.logFileStream) {
      this.smaugSvc.logFileStream.write(datStr, datStr.length);
      this.smaugSvc.logFileStream.write(str, str.length);
    }
  },

  DEBUG_LOG: function (str)
  {
    if ((gLogLevel >= 4) || (this.smaugSvc && this.smaugSvc.logFileStream))
  ...
  getString: function (aStr, subPhrases)
  {
    ...
    if (!this.smgStringBundle) {
      try {
        ...
        this.smgStringBundle = strBundleService.createBundle("chrome://smaug/locale/smaug.properties");
      }
      catch (ex) {
        this.ERROR_LOG("smaugCommon.jsm: Error in instantiating stringBundleService\n");
      }
    }
      ...
      try {
        if (subPhrases) {
          if (typeof(subPhrases) == "string") {
            return this.smgStringBundle.formatStringFromName(aStr, [ subPhrases ], 1);
          }
          else
            return this.smgStringBundle.formatStringFromName(aStr, subPhrases, subPhrases.length);
        }
        else {
          return this.smgStringBundle.GetStringFromName(aStr);
        }
      }
      catch (ex) {
        this.ERROR_LOG("smaugCommon.jsm: Error in querying stringBundleService for string '"+aStr+"'\n");
      }
    }
    return aStr;
  },
  ...
  {
    //this.DEBUG_LOG("smaugCommon.jsm: convertToUnicode: "+charset+"\n");
  ...
  convertFromUnicode: function (text, charset) {
    //this.DEBUG_LOG("smaugCommon.jsm: convertFromUnicode: "+charset+"\n");
    ...
    try {
      ...
      this.DEBUG_LOG("smaugCommon.jsm: convertFromUnicode: caught an exception\n");
  ...
  {
    this.DEBUG_LOG("smaugCommon.jsm: parseErrorOutput: status message: \n"+errOutput+"\n");
      ...
      if (errLines[j].search(statusPat) == 0) {
        ...
        if (matches && (matches.length > 1)) {
          ...
          if (flag == Ci.nsIEnigmail.DECRYPTION_FAILED) {
            ...
            flag = 0;
            this.DEBUG_LOG("smaugCommon.jsm: parseErrorOutput: detected invalid sender: "+lineSplit[2]+" / code: "+lineSplit[1]+"\n");
    ...
    this.DEBUG_LOG("smaugCommon.jsm: parseErrorOutput: statusFlags = "+this.bytesToHex(this.pack(statusFlags,4))+"\n");
    ...
    if (encryptToArray.length > 0) {
      ...
      for (var encIdx=0; encIdx<encryptToArray.length; ++encIdx) {
        ...
        if (keyId != "0x0000000000000000") {
          var userId = this.smaugSvc.getFirstUserIdOfKey(keyId);
          if (userId) {
  ...
  initialize: function (smaugSvc, logLevel)
  {
    this.smaugSvc = smaugSvc;
    gLogLevel = logLevel;
  },
  ...
  {
    this.DEBUG_LOG("smaugCommon.jsm: dispatchEvent f="+callbackFunction.name+"\n");
    ...
    mainEvent.prototype = {
      ...
      run: function()
      {
        SmaugCommon.DEBUG_LOG("smaugCommon.jsm: dispatchEvent running mainEvent\n");
        this.cbFunc(this.args);
      },

      notify: function()
      {
        SmaugCommon.DEBUG_LOG("smaugCommon.jsm: dispatchEvent got notified\n");
        this.cbFunc(this.args);
      }
  ...
  rememberEncryptedUri: function (uri) {
    this.DEBUG_LOG("smaugCommon.jsm: rememberEncryptedUri: uri="+uri+"\n");
  ...
  forgetEncryptedUri: function (uri) {
    this.DEBUG_LOG("smaugCommon.jsm: forgetEncryptedUri: uri="+uri+"\n");
  ...
  isEncryptedUri: function (uri) {
    this.DEBUG_LOG("smaugCommon.jsm: isEncryptedUri: uri="+uri+"\n");
  ...
  fixExitCode: function (exitCode, statusFlags) {
    if (exitCode != 0) {
      if ((statusFlags & (nsIEnigmail.BAD_PASSPHRASE | nsIEnigmail.UNVERIFIED_SIGNATURE)) &&
        ...
        this.DEBUG_LOG("smaugCommon.jsm: Smaug.fixExitCode: Changing exitCode for decrypted msg "+exitCode+"->0\n");
        exitCode = 0;
      }
    }

    if ((this.smaugSvc.agentType == "gpg") && (exitCode == 256) && (this.getOS() == "WINNT")) {
      this.WARNING_LOG("smaugCommon.jsm: Smaug.fixExitCode: Using gpg and exit code is 256. You seem to use cygwin-gpg, activating countermeasures.\n");
      if (statusFlags & (nsIEnigmail.BAD_PASSPHRASE | nsIEnigmail.UNVERIFIED_SIGNATURE)) {
        this.WARNING_LOG("smaugCommon.jsm: Smaug.fixExitCode: Changing exitCode 256->2\n");
        exitCode = 2;
      } else {
        this.WARNING_LOG("smaugCommon.jsm: Smaug.fixExitCode: Changing exitCode 256->0\n");
        exitCode = 0;
      }
    }
    if ((this.smaugSvc.agentVersion < "1.4.1") && this.isDosLike()) {
        ...
        this.WARNING_LOG("smaugCommon.jsm: Smaug.fixExitCode: Using gpg version "+this.smaugSvc.agentVersion+", activating countermeasures for file renaming bug.\n");
        exitCode = 0;
      }
    }
    return exitCode;
  ...
  generateKey: function (parent, name, comment, email, expiryDate, keyLength, keyType,
            passphrase, listener) {
    this.WRITE_LOG("smaugCommon.jsm: generateKey:\n");
    ...
    this.CONSOLE_LOG(this.printCmdLine(this.smaugSvc.agentPath, args));
    ...
    try {
      proc = subprocess.call({
        command:     this.smaugSvc.agentPath,
        arguments:   args,
      ...
      this.ERROR_LOG("smaugCommon.jsm: generateKey: subprocess.call failed with '"+ex.toString()+"'\n");
      throw ex;
    }

    gKeygenProcess = proc;

    this.DEBUG_LOG("smaugCommon.jsm: generateKey: subprocess = "+proc+"\n");
  ...
  {
    this.DEBUG_LOG("smaugCommon.jsm: newStringChannel\n");
    ...
    this.DEBUG_LOG("smaugCommon.jsm: newStringChannel - done\n");
  ...
  getGpgGroups: function() {
    if (!this.smaugSvc) return [];

    let exitCodeObj = {};
    let errorMsgObj = {};

    let cfgStr = this.smaugSvc.getGnupgConfig(exitCodeObj, errorMsgObj);
  ...
  getSecretKeys: function (win) {
    ...
    var smaugSvc = this.getService(win);
    if (!smaugSvc) {
      return null;
    }
    ...
    var keyList=smaugSvc.getUserIdList(true, false, exitCodeObj, statusFlagsObj, errorMsgObj);
    ...
    keyList = smaugSvc.getKeyDetails(secretKeyList.join(" "), false);
  ...
  receiveKey: function (recvFlags, keyserver, keyId, listener, errorMsgObj) {
    this.DEBUG_LOG("smaugCommon.jsm: receiveKey: "+keyId+"\n");

    if (! (this.smaugSvc && this.smaugSvc.initialized)) {
      this.ERROR_LOG("smaugCommon.jsm: receiveKey: not yet initialized\n");
    ...
    this.CONSOLE_LOG("smaug> "+this.printCmdLine(this.smaugSvc.agentPath, args)+"\n");
    ...
    try {
      ...
      this.ERROR_LOG("smaugCommon.jsm: receiveKey: subprocess.call failed with '"+ex.toString()+"'\n");
      throw ex;
    }

    if (!proc) {
      this.ERROR_LOG("smaugCommon.jsm: receiveKey: subprocess failed due to unknown reasons\n");
      return null;
    }
  ...
  searchKey: function (recvFlags, protocol, keyserver, port, keyValue, listener, errorMsgObj) {
    this.DEBUG_LOG("smaugCommon.jsm: Smaug.searchKey: "+keyValue+"\n");

    if (! (this.smaugSvc && this.smaugSvc.initialized)) {
      this.ERROR_LOG("smaugCommon.jsm: searchKey: not yet initialized\n");
      errorMsgObj.value = this.getString("notInit");
      return null;
    }
    ...
    try {
      ...
      this.ERROR_LOG("smaugCommon.jsm: searchKey: subprocess.call failed with '"+ex.toString()+"'\n");
      throw ex;
    }

    if (!proc) {
      this.ERROR_LOG("smaugCommon.jsm: searchKey: subprocess failed due to unknown reasons\n");
      return null;
    }
  ...
  haveCachedPassphrase: function () {
    this.DEBUG_LOG("smaugCommon.jsm: haveCachedPassphrase: \n");
  ...
  //  this.DEBUG_LOG("currentTime="+currentTime+", _lastActiveTime="+this._lastActiveTime+", expired="+expired+"\n");
    ...
    if (expired && (_passwdAccessTimes <= 0)) {
      ...
      this.WRITE_LOG("smaugCommon.jsm: haveCachedPassphrase: CACHE IDLED OUT\n");
    }
  ...
  stillActive: function () {
    this.DEBUG_LOG("smaugCommon.jsm: stillActive: \n");

    // Update last active time
    var curDate = new Date();
    _lastActiveTime = curDate.getTime();
    // this.DEBUG_LOG("smaugCommon.jsm: stillActive: _lastActiveTime="+this._lastActiveTime+"\n");
  },

  clearCachedPassphrase: function () {
    this.DEBUG_LOG("smaugCommon.jsm: clearCachedPassphrase: \n");

    gCachedPassphrase = null;
  },

  setCachedPassphrase: function (passphrase) {
    this.DEBUG_LOG("smaugCommon.jsm: setCachedPassphrase: \n");
    ...
    if (createTimerType != null) {
      ...
      if (!gCacheTimer) {
        this.ERROR_LOG("smaugCommon.jsm: setCachedPassphrase: Error - failed to create timer\n");
        throw Components.results.NS_ERROR_FAILURE;
      }
      ...
      this.DEBUG_LOG("smaugCommon.jsm: setCachedPassphrase: gCacheTimer="+gCacheTimer+"\n");
    }
  },

  getPassphrase: function (domWindow, passwdObj, useAgentObj, rememberXTimes) {

    this.DEBUG_LOG("smaugCommon.jsm: getPassphrase:\n");

    useAgentObj.value = false;
    try {
      var noPassphrase = this.getPref("noPassphrase");
      useAgentObj.value = this.smaugSvc.useGpgAgent();
    ...
    this.DEBUG_LOG("smaugCommon.jsm:: GetPassphrase: got passphrase\n");
  ...
  clearPassphrase: function (win) {
    this.DEBUG_LOG("smaugCommon.jsm: clearPassphrase:\n");

    var smaugSvc = this.getService(win);
    if (!smaugSvc)
      return;

    if (smaugSvc.useGpgAgent(win)) {
  ...
  extractPubkey: function (statusMsg) {
    ...
    if (matchb && (matchb.length > 3)) {
      this.DEBUG_LOG("smaugCommon.jsm:: Smaug.extractPubkey: NO_PUBKEY 0x"+matchb[3]+"\n");
  ...
  execStart: function (command, args, needPassphrase, domWindow, listener, statusFlagsObj) {
    this.WRITE_LOG("smaugCommon.jsm: execStart: command = "+this.printCmdLine(command, args)+", needPassphrase="+needPassphrase+", domWindow="+domWindow+", listener="+listener+"\n");
    ...
    if (needPassphrase) {
      ...
      if (!this.getPassphrase(domWindow, passwdObj, useAgentObj, 0)) {
         this.ERROR_LOG("smaugCommon.jsm: execStart: Error - no passphrase supplied\n");
    ...
    this.CONSOLE_LOG("smaug> "+this.printCmdLine(command, args)+"\n");
      ...
      proc = subprocess.call({
        ...
        stdin: function (pipe) {
          if (needPassphrase) {
            ...
            try {
              if (SmaugCommon.requirePassword()) {
        ...
        stderr: function(data) { listener.stderr(data); },
          ...
          try {
            ...
            SmaugCommon.writeException("smaugCommon.jsm", ex);
          }
        },
        mergeStderr: false
      });
    } catch (ex) {
      this.ERROR_LOG("smaugCommon.jsm: execStart: subprocess.call failed with '"+ex.toString()+"'\n");
      return null;
    }
  ...
  execEnd: function (listener, statusFlagsObj, statusMsgObj, cmdLineObj, errorMsgObj, blockSeparationObj) {
    ...
    this.DEBUG_LOG("smaugCommon.jsm: execEnd:\n");
    ...
    this.DEBUG_LOG("smaugCommon.jsm: execEnd: exitCode = "+exitCode+"\n");
    this.DEBUG_LOG("smaugCommon.jsm: execEnd: errOutput = "+errOutput+"\n");
  ...
  decryptMessageStart: function (win, verifyOnly, noOutput, listener,
                                 ...
                                 maxOutputLength) {
    this.DEBUG_LOG("smaugCommon.jsm: decryptMessageStart: verifyOnly="+verifyOnly+"\n");

    this.getService(win);
    if (! (this.smaugSvc)) {
      this.ERROR_LOG("smaugCommon.jsm: decryptMessageStart: not yet initialized\n");
      errorMsgObj.value = this.getString("notInit");
      return null;
    }
    ...
    var proc = this.execStart(this.smaugSvc.agentPath, args, !verifyOnly, win,
                              listener, statusFlagsObj);

    if (statusFlagsObj.value & nsIEnigmail.MISSING_PASSPHRASE) {
      this.ERROR_LOG("smaugCommon.jsm: decryptMessageStart: Error - no passphrase supplied\n");
  ...
  {
    this.DEBUG_LOG("smaugCommon.jsm: decryptMessageEnd: uiFlags="+uiFlags+", verifyOnly="+verifyOnly+", noOutput="+noOutput+"\n");

    stderrStr = stderrStr.replace(/\r\n/g,"\n");
    this.DEBUG_LOG("smaugCommon.jsm: decryptMessageEnd: stderrStr=\n"+stderrStr+"\n");
    ...
    if (userId && keyId && this.prefBranch.getBoolPref("displaySecondaryUid")) {
      let uids = this.smaugSvc.getKeyDetails(keyId, true, true);
      if (uids) {
        userId = uids;
      }
    ...
    if (exitCode != 0) {
      // Error processing
      this.DEBUG_LOG("smaugCommon.jsm: decryptMessageEnd: command execution exit code: "+exitCode+"\n");
    }
  ...
  getEncryptCommand: function (fromMailAddr, toMailAddr, bccMailAddr, hashAlgorithm, sendFlags, isAscii, errorMsgObj) {
    this.DEBUG_LOG("smaugCommon.jsm: getEncryptCommand: hashAlgorithm="+hashAlgorithm+"\n");
    this.DEBUG_LOG("smaugCommon.jsm: getEncryptCommand: fromMailAddr="+fromMailAddr+", toMailAddr="+toMailAddr+", bccMailAddr="+bccMailAddr+"\n");
    ...
    if (encryptMsg) {
      ...
      if (sendFlags & nsIEnigmail.SEND_ALWAYS_TRUST) {
        if (this.smaugSvc.agentVersion >= "1.4") {
  ...
  determineHashAlgorithm: function (win, uiFlags, fromMailAddr, hashAlgoObj) {
    this.DEBUG_LOG("smaugCommon.jsm: determineHashAlgorithm\n");
    ...
    this.getService(win);
    if (! (this.smaugSvc)) {
      this.ERROR_LOG("smaugCommon.jsm: determineHashAlgorithm: not yet initialized\n");
      errorMsgObj.value = this.getString("notInit");
      return 2;
    }
    ...
    if (typeof(gKeyAlgorithms[fromMailAddr]) != "string") {
      ...
      if (!this.getPassphrase(null, passwdObj, useAgentObj, 2)) {
        this.ERROR_LOG("smaugCommon.jsm: determineHashAlgorithm: Error - no passphrase supplied\n");
      ...
      if (m && (m.length > 2) && (m[1] == "Hash: ")) {
        ...
        this.DEBUG_LOG("smaugCommon.jsm: determineHashAlgorithm: no hashAlgorithm specified - using MD5\n");

      for (var i=1; i < gMimeHashAlgorithms.length; i++) {
        if (gMimeHashAlgorithms[i] == hashAlgorithm) {
          this.DEBUG_LOG("smaugCommon.jsm: determineHashAlgorithm: found hashAlgorithm "+hashAlgorithm+"\n");
      ...
      this.ERROR_LOG("smaugCommon.jsm: determineHashAlgorithm: no hashAlgorithm found\n");
      return 2;
    }
    else {
      this.DEBUG_LOG("smaugCommon.jsm: determineHashAlgorithm: hashAlgorithm "+gKeyAlgorithms[fromMailAddr]+" is cached\n");
  ...
  encryptMessageStart: function (win, uiFlags, fromMailAddr, toMailAddr, bccMailAddr,
    ...
    this.DEBUG_LOG("smaugCommon.jsm: encryptMessageStart: uiFlags="+uiFlags+", from "+fromMailAddr+" to "+toMailAddr+", hashAlgorithm="+hashAlgorithm+" ("+this.bytesToHex(this.pack(sendFlags,4))+")\n");
    ...
    if (!sendFlags) {
      this.DEBUG_LOG("smaugCommon.jsm: encryptMessageStart: NO ENCRYPTION!\n");
    ...
    this.getService(win);
    if (! (this.smaugSvc)) {
      this.ERROR_LOG("smaugCommon.jsm: encryptMessageStart: not yet initialized\n");
      errorMsgObj.value = this.getString("notInit");
      return null;
    }
    ...
    var proc = this.execStart(this.smaugSvc.agentPath, encryptArgs, signMsg, win, listener, statusFlagsObj);

    if (statusFlagsObj.value & nsIEnigmail.MISSING_PASSPHRASE) {
      this.ERROR_LOG("smaugCommon.jsm: encryptMessageStart: Error - no passphrase supplied\n");
  ...
  encryptMessageEnd: function (stderrStr, exitCode, uiFlags, sendFlags, outputLen,
            retStatusObj)
  {
    this.DEBUG_LOG("smaugCommon.jsm: encryptMessageEnd: uiFlags="+uiFlags+", sendFlags="+this.bytesToHex(this.pack(sendFlags,4))+", outputLen="+outputLen+"\n");
    ...
    if (!this.smaugSvc.initialized) {
       this.ERROR_LOG("smaugCommon.jsm: encryptMessageEnd: not yet initialized\n");
    ...
    // Error processing
    this.DEBUG_LOG("smaugCommon.jsm: encryptMessageEnd: command execution exit code: "+exitCode+"\n");
  ...
  getAttachmentFileName: function (parent, byteData) {
    this.DEBUG_LOG("smaugCommon.jsm: getAttachmentFileName\n");
    ...
    if (!this.getPassphrase(parent, passwdObj, useAgentObj, 0)) {
      this.ERROR_LOG("smaugCommon.jsm: getAttachmentFileName: Error - no passphrase supplied\n");
      return null;
    }
    ...
    var listener = this.newSimpleListener(
      function _stdin (pipe) {
          SmaugCommon.DEBUG_LOG("smaugCommon.jsm: getAttachmentFileName: _stdin\n");
          if (SmaugCommon.requirePassword()) {
            pipe.write(passphrase+"\n");
          }
          pipe.write(byteData);
    ...
    var statusFlagsObj = {};
    var proc = this.execStart(this.smaugSvc.agentPath, args, false, parent,
                              listener, statusFlagsObj);
...
var timerObserver = {
  ...
  observe: function (aSubject, aTopic, aData) {
    SmaugCommon.DEBUG_LOG("smaugCommon.jsm: timerObserver.observe: topic='"+aTopic+"' \n");

    if (aTopic == "timer-callback") {
      // Cause cached password to expire, if need be
      if (!SmaugCommon.haveCachedPassphrase()) {
      ...
      SmaugCommon.DEBUG_LOG("smaugCommon.jsm: timerObserver.observe: no handler for '"+aTopic+"'\n");
    }
  }
};
...
function upgradeRecipientsSelection () {
  ...
  var  keySel = SmaugCommon.getPref("recipientsSelectionOption");
  var  perRecipientRules = SmaugCommon.getPref("perRecipientRules");
  ...
  */
  ...
  // set new pref
  SmaugCommon.setPref("recipientsSelection", setVal);

  // clear old prefs
  SmaugCommon.prefBranch.clearUserPref("perRecipientRules");
  SmaugCommon.prefBranch.clearUserPref("recipientsSelectionOption");
}


function upgradePrefsSending ()
{
  SmaugCommon.DEBUG_LOG("smaugCommon.jsm: upgradePrefsSending()\n");

  var  cbs = SmaugCommon.getPref("confirmBeforeSend");
  var  ats = SmaugCommon.getPref("alwaysTrustSend");
  var  ksfr = SmaugCommon.getPref("keepSettingsForReply");
  SmaugCommon.DEBUG_LOG("smaugCommon.jsm: upgradePrefsSending cbs="+cbs+" ats="+ats+" ksfr="+ksfr+"\n");

  // Upgrade confirmBeforeSend (bool) to confirmBeforeSending (int)
  switch (cbs) {
    case false:
      SmaugCommon.setPref("confirmBeforeSending", 0); // never
      break;
    case true:
      SmaugCommon.setPref("confirmBeforeSending", 1); // always
      break;
  }
  ...
  switch (ats) {
    case false:
      SmaugCommon.setPref("acceptedKeys", 0); // valid
      break;
    case true:
      SmaugCommon.setPref("acceptedKeys", 1); // all
      break;
  }

  // if all settings are default settings, use convenient encryption
  if (cbs==false && ats==true && ksfr==true) {
    SmaugCommon.setPref("encryptionModel", 0); // convenient
    SmaugCommon.DEBUG_LOG("smaugCommon.jsm: upgradePrefsSending() encryptionModel=0 (convenient)\n");
  }
  else {
    SmaugCommon.setPref("encryptionModel", 1); // manually
    SmaugCommon.DEBUG_LOG("smaugCommon.jsm: upgradePrefsSending() encryptionModel=1 (manually)\n");
  }

  // clear old prefs
  SmaugCommon.prefBranch.clearUserPref("confirmBeforeSend");
  SmaugCommon.prefBranch.clearUserPref("alwaysTrustSend");
}
...
function stripEmailAdr(mailAddrs) {
  ...
  while ((qStart = mailAddrs.indexOf('"')) != -1) {
     ...
     if (qEnd == -1) {
       this.ERROR_LOG("smaugCommon.jsm:: stripEmailAdr: Unmatched quote in mail address: "+mailAddrs+"\n");
...
function upgradeHeadersView() {
  ...
  var hdrMode = null;
  try {
    hdrMode = SmaugCommon.getPref("show_headers");
  }
  catch (ex) {}

  if (hdrMode == null) hdrMode = 1;
  try {
    SmaugCommon.prefBranch.clearUserPref("show_headers");
  }
  catch (ex) {}

  SmaugCommon.prefRoot.setIntPref("mail.show_headers", hdrMode);
}

function upgradeCustomHeaders() {
  try {
    var extraHdrs = " " + SmaugCommon.prefRoot.getCharPref("mailnews.headers.extraExpandedHeaders").toLowerCase() + " ";
    ...
    SmaugCommon.prefRoot.setCharPref("mailnews.headers.extraExpandedHeaders", extraHdrs);
  }
  catch(ex) {}
}
...
function upgradePgpMime() {
  ...
  try {
    pgpMimeMode = (SmaugCommon.getPref("usePGPMimeOption") == 2);
  }
  catch (ex) {
    return;
  }
  ...
  try {
    ...
    SmaugCommon.prefBranch.clearUserPref("usePGPMimeOption");
  }
  catch (ex) {}
}


function ConfigureSmaug(window, startingPreferences) {
  SmaugCommon.DEBUG_LOG("smaugCommon.jsm: ConfigureSmaug\n");
  var oldVer=SmaugCommon.getPref("configuredVersion");
  ...
  try {
    ...
    if (oldVer == "") {
      ...
      if (vc.compare(oldVer, "1.7a1pre") < 0) {
        // MISSING:
        // - upgrade extensions.smaug.recipientsSelection
        //   to      extensions.smaug.assignKeys*
        // 1: rules only
      ...
      if (vc.compare(oldVer, "1.7") < 0) {
        ...
        SmaugCommon.setTimeout(
          function _cb() {
            var doIt = SmaugCommon.confirmDlg(window,
                                   SmaugCommon.getString("smaugCommon.versionSignificantlyChanged"),
                                   SmaugCommon.getString("smaugCommon.checkPreferences"),
                                   SmaugCommon.getString("dlg.button.close"));
            if (!startingPreferences && doIt) {
                // same as:
                // - SmaugFuncs.openPrefWindow(window, true, 'sendingTab');
                // but
                // - without starting the service again because we do that right now
                // - and modal (waiting for its end)
                window.openDialog("chrome://smaug/content/pref-smaug.xul",
...
try {
  AddonManager.getAddonByID(SMG_EXTENSION_GUID,
    function (addon) {
      gSmgExtensionVersion = addon.version;
      gSmgInstallLocation = addon.getResourceURI("").QueryInterface(Ci.nsIFileURL).file;
      initSubrocess(gSmgInstallLocation);
    }
  );

}
catch (ex) {
  dump("smaugCommon.jsm: init error: "+ex+"\n");
}
