...
/*
 * Common Smaug crypto-related GUI functionality
 *
...
var EXPORTED_SYMBOLS = [ "SmaugFuncs" ];
...
//      see validKeysForAllRecipients() in smaugMsgComposeHelper.js
//  ---------------------------------------------------------
...
var SmaugFuncs = {
  ...
  {
    SmaugCommon.DEBUG_LOG("commonFuncs.jsm: downloadKeys: searchList="+inputObj.searchList+"\n");

    resultObj.importedKeys=0;

    var ioService = Cc[IOSERVICE_CONTRACTID].getService(Ci.nsIIOService);
    if (ioService && ioService.offline) {
      SmaugCommon.alert(win, SmaugCommon.getString("needOnline"));
      return;
    }
    ...
    win.openDialog("chrome://smaug/content/smaugKeyserverDlg.xul",
    ...
    win.openDialog("chrome://smaug/content/smaugSearchKey.xul",
  ...
  stripEmail: function (mailAddrs)
  {
    ...
    while ((qStart = mailAddrs.indexOf('"')) != -1) {
       ...
       if (qEnd == -1) {
         SmaugCommon.ERROR_LOG("commonFuncs.jsm: stripEmail: Unmatched quote in mail address: "+mailAddrs+"\n");
  ...
  {
    SmaugCommon.DEBUG_LOG("commonFuncs.jsm: collapseAdvanced:\n");

    var advancedUser = SmaugCommon.getPref("advancedUser");
  ...
  {
     win.open("chrome://smaug/content/smaugSetupWizard.xul",
                "", "chrome,centerscreen");
  },
  ...
  {
    SmaugCommon.openWin("smaug:help",
                           "chrome://smaug/content/smaugHelp.xul?src="+source,
                           "centerscreen,resizable");
  },

  /**
   * Display the "About Smaug" window
   *
   * no return value
   */
  ...
  {
    SmaugCommon.openWin("about:smaug",
                           "chrome://smaug/content/smaugAbout.xul",
  ...
  {
    SmaugCommon.openWin("smaug:rulesEditor",
                           "chrome://smaug/content/smaugRulesEditor.xul",
  ...
  {
    SmaugCommon.getService(win);

    SmaugCommon.openWin("smaug:KeyManager",
                           "chrome://smaug/content/smaugKeyManager.xul",
                           "resizable");
  },
  ...
  openKeyGen: function ()
  {
    SmaugCommon.openWin("smaug:generateKey",
                           "chrome://smaug/content/smaugKeygen.xul",
  ...
  {
    SmaugCommon.openWin("smaug:cardDetails",
                           "chrome://smaug/content/smaugCardDetails.xul",
                           "centerscreen");
  },
  ...
  {
     SmaugCommon.openWin("smaug:console",
                            "chrome://smaug/content/smaugConsole.xul",
                            "resizable,centerscreen");
  },
  ...
  {
    var logDirectory = SmaugCommon.getPref("logDirectory");

    if (!logDirectory) {
      SmaugCommon.alert(win, SmaugCommon.getString("noLogDir"));
      return;
    }

    var svc = SmaugCommon.smaugSvc;
    if (! svc) {
      SmaugCommon.alert(win, SmaugCommon.getString("noLogFile"));
      return;
    }

    if (! svc.logFileStream) {
      SmaugCommon.alert(win, SmaugCommon.getString("restartForLog"));
      return;
    }
    ...
    var logFileURL = "file:///" + logDirectory + "/smgdbug.txt";
    var opts="fileUrl=" + escape(logFileURL) + "&title=" +
          escape(SmaugCommon.getString("debugLog.title"));

    SmaugCommon.openWin("smaug:logFile",
                           "chrome://smaug/content/smaugViewFile.xul?"+opts,
                           "resizable,centerscreen");
  },
  ...
  {
    SmaugCommon.DEBUG_LOG("smaugCommon.js: prefWindow\n");

    SmaugCommon.getService(win,true);  // true: starting preferences dialog

    win.openDialog("chrome://smaug/content/pref-smaug.xul",
  ...
  createNewRule: function (win, emailAddress)
  {
    ...
    var smaugSvc = SmaugCommon.getService(win);
    if (!smaugSvc) return false;

    var rulesListObj= new Object;

    // open rule dialog
    smaugSvc.getRulesData(rulesListObj);
    var inputObj=new Object;
    var resultObj=new Object;
    inputObj.toAddress="{"+emailAddress+"}";
    inputObj.options="";
    inputObj.command = "add";
    win.openDialog("chrome://smaug/content/smaugSingleRcptSettings.xul","",
  ...
  editKeyExpiry: function (win, userIdArr, keyIdArr)
  {
    ...
    win.openDialog("chrome://smaug/content/smaugEditKeyExpiryDlg.xul","",
                   "dialog,modal,centerscreen,resizable", inputObj, resultObj);
  ...
  editKeyTrust: function (win, userIdArr, keyIdArr)
  {
    ...
    var resultObj = { refresh: false };
    win.openDialog("chrome://smaug/content/smaugEditKeyTrustDlg.xul","",
                   "dialog,modal,centerscreen,resizable", inputObj, resultObj);
    return resultObj.refresh;
  },
  ...
  signKey: function (win, userId, keyId)
  {
    ...
    var resultObj = { refresh: false };
    win.openDialog("chrome://smaug/content/smaugSignKeyDlg.xul","",
                   "dialog,modal,centerscreen,resizable", inputObj, resultObj);
    return resultObj.refresh;
  },
  ...
  showPhoto: function (win, keyId, userId, photoNumber)
  {
    ...
    if (smaugSvc) {
      ...
      var photoPath = smaugSvc.showKeyPhoto("0x"+keyId, photoNumber, exitCodeObj, errorMsgObj);

      if (photoPath && exitCodeObj.value==0) {

        var photoFile = Cc[SmaugCommon.LOCAL_FILE_CONTRACTID].
          createInstance(Ci.nsIFile);
        photoFile.initWithPath(photoPath);
        if (! (photoFile.isFile() && photoFile.isReadable())) {
          SmaugCommon.alert(win, SmaugCommon.getString("error.photoPathNotReadable", photoPath));
        }
        else {
          var ioServ = Cc[SmaugCommon.IOSERVICE_CONTRACTID].getService(Ci.nsIIOService);
          ...
          win.openDialog("chrome://smaug/content/smaugDispPhoto.xul",
                         photoUri,
        ...
        SmaugCommon.alert(win, SmaugCommon.getString("noPhotoAvailable"));
      }
    }
  },
  ...
  openKeyDetails: function (win, keyId, refresh)
  {
    ...
    var resultObj = { refresh: false };
    win.openDialog("chrome://smaug/content/smaugKeyDetailsDlg.xul", "",
                   "dialog,modal,centerscreen,resizable", inputObj, resultObj);
  ...
  {
    SmaugCommon.DEBUG_LOG("smaugFuncs.jsm: loadKeyList\n");
    ...
    var sortByValidity = function (a, b) {
      return (TRUSTLEVELS_SORTED.indexOf(SmaugFuncs.getTrustCode(keyListObj.keyList[a.keyId])) < TRUSTLEVELS_SORTED.indexOf(SmaugFuncs.getTrustCode(keyListObj.keyList[b.keyId]))) ? -sortDirection : sortDirection;
    };
    ...
    if (!aGpgSecretsList && !refresh) {
      if (SmaugCommon.confirmDlg(SmaugCommon.getString("noSecretKeys"),
            SmaugCommon.getString("keyMan.button.generateKey"),
            SmaugCommon.getString("keyMan.button.skip"))) {
        this.openKeyGen();
    ...
    for (i=0; i<aGpgUserList.length; i++) {
      ...
      if (listRow.length>=0) {
        ...
        case "pub":
          keyObj = new Object();
          uatNum = 0;
          keyObj.expiry=SmaugCommon.getDateTime(listRow[EXPIRY_ID], true, false);
          keyObj.expiryTime = Number(listRow[EXPIRY_ID]);
          keyObj.created=SmaugCommon.getDateTime(listRow[CREATED_ID], true, false);
        ...
        case "uid":
          ...
          if (typeof(keyObj.userId) != "string") {
            keyObj.userId=SmaugCommon.convertGpgToUnicode(listRow[USERID_ID]);
            ...
            var subUserId = {
              userId: SmaugCommon.convertGpgToUnicode(listRow[USERID_ID]),
        ...
        case "uat":
          if (listRow[USERID_ID].indexOf("1 ")==0) {
            var userId=SmaugCommon.getString("userAtt.photo");
  ...
  {
    SmaugCommon.DEBUG_LOG("smaugFuncs.jsm: obtainKeyList\n");
    ...
    try {
      ...
      var smaugSvc = SmaugCommon.getService(win);
      if (! smaugSvc) return null;

      userList = smaugSvc.getUserIdList(secretOnly,
                                           refresh,
                                           exitCodeObj,
                                           statusFlagsObj,
                                           errorMsgObj);
      if (exitCodeObj.value != 0) {
        SmaugCommon.alert(win, errorMsgObj.value);
        return null;
      }
    } catch (ex) {
      SmaugCommon.ERROR_LOG("ERROR in smaugFuncs: obtainKeyList"+ex.toString()+"\n");
    }
   ...
   * Translates "old-style" defaults (pre-Smaug v1.0) to "current" defaults
   *
  ...
  {
    SmaugCommon.DEBUG_LOG("smaugFuncs.jsm: getSignMsg: identity.key="+identity.key+"\n");
    var sign = null;

    SmaugCommon.getPref("configuredVersion"); // dummy call to getPref to ensure initialization

    var prefRoot = SmaugCommon.prefRoot;
  ...
  formatPlaintextMsg: function (plainTxt)
  {
    ...
    if (! SmaugCommon.prefRoot)
      SmaugCommon.getPref("configuredVersion");
    ...
    //SmaugCommon.DEBUG_LOG("smaugFuncs.jsm: r='"+r+"'\n");
    return r;
  },
  ...
  getHeaderData: function (data) {
    SmaugCommon.DEBUG_LOG("smaugFuncs.jsm: getHeaderData: "+data.substr(0, 100)+"\n");
    var a = data.split(/\n/);
    var res = [];
    for (let i = 0; i < a.length; i++) {
      ...
      for (let j=0; j < b.length; j++) {
        ...
        if (m) {
          ...
          SmaugCommon.DEBUG_LOG("smaugFuncs.jsm: getHeaderData: "+m[2].toLowerCase()+" = "+res[m[2].toLowerCase()] +"\n");
        }
      }
  ...
  writeFileContents: function(filePath, data, permissions) {
    ...
    // SmaugCommon.DEBUG_LOG("smaugFuncs.jsm: WriteFileContents: file="+filePath.toString()+"\n");
    ...
    try {
      ...
      SmaugCommon.ERROR_LOG("smaugFuncs.jsm: writeFileContents: Failed to write to "+filePath+"\n");
      return false;
    }
  ...
  createFileStream: function(filePath, permissions) {
    //SmaugCommon.DEBUG_LOG("smaugFuncs.jsm: createFileStream: file="+filePath+"\n");
    ...
    try {
      ...
      SmaugCommon.ERROR_LOG("smaugFuncs.jsm: CreateFileStream: Failed to create "+filePath+"\n");
      return null;
    }
  }
